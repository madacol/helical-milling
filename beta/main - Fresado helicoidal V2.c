...
#define I_buttons		RA4
#define I_enc_1			RE0
#define I_enc_2			RE1
// #define 		RB0
#define O_disp_1		RB1
#define butt_modo		RB1
#define O_disp_2		RB2
#define butt_1			RB2
#define O_disp_3		RB3
#define butt_2			RB3
...
#define _step_accel			0.001 			// (#Pasos por milisegundo) por milisegundo
...
// Configuracion inicial del PIC
	...
	TRISB = 0b00000001;		// CONFIGURA RB0 COMO ENTRADA. Y LAS DEMAS COMO SALIDAS.
	TRISA = 0b110000;		// CONFIGURA RA4 y RA5 COMO ENTRADA. Y LAS DEMAS COMO SALIDAS.
	TRISE = 0b111;			// CONFIGURA PORTE COMO ENTRADA.
...
while (1) {
	...
	if (modo == 0) {
		...
		if (last_enc != now_enc) {
			...
			if (enc_step_cont <= -0.5) {
				--int_enc_step_cont;
				enc_step_cont += 1;
			}
			else if (enc_step_cont >= 0.5) {
				++int_enc_step_cont;
				enc_step_cont -= 1;
			}		
		...
		if (active_button > 0) {			
			if (holding) {
				...
				if (active_button == 1) {
					...
					i = 0;
					led_prog = 1;
					steps_per_diente = 28800 / num_dientes;
